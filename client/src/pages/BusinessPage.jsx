...
const BusinessPage = () => {
  ...
  const [notifications, setNotifications] = useState([]); // NEW: Page Notifications
  ...
  const [newItem, setNewItem] = useState({ title: '', price: '', currency: 'BDT', condition: 'New', description: '', category: 'Digital Products', location: 'Dhaka', address: '', stock: 1, shippingType: 'Shop Pickup', returnPolicy: 'No Return' });
  ...
  const loadPageData = (pageId, currentUser) => {
    ...
    const unsubInbox = onSnapshot(qInbox, (snap) => {
        ...
        // Filter out conversations that involve two businesses (or edge cases) if necessary, but keep all for simplicity here
    ...
    // 4. NEW: Page Notifications (Reviews/Questions)
  ...
  const handleInviteMember = async () => {
      ...
      await addDoc(collection(db, 'notifications'), {
            ...
            senderId: user.uid, // The user performing the action
            senderName: page.name,
            senderAvatar: page.logo,
            type: 'team_invite',
            pageId: id, // Critical for acceptance
            pageName: page.name,
            role: newMemberRole, // Critical for permission
  ...
  const handleNotificationAction = async (notif) => {
      // Mark as read (optional, but good practice for internal notifications)
      await updateDoc(doc(db, 'notifications', notif.id), { isRead: true });
      // Navigate to product details to reply/view
      if (notif.targetId) {
  ...
  const handleSaveProduct = async () => { if (mediaFiles.length === 0 && existingMedia.length === 0) return alert("Image required."); setProductUploading(true); try { const newMedia = await Promise.all(mediaFiles.map(async (file) => { const refS = ref(storage, `business/${id}/products/${Date.now()}_${file.name}`); await uploadBytes(refS, file); return { url: await getDownloadURL(refS), type: file.type.startsWith('video') ? 'video' : 'image' }; })); const finalMedia = [...existingMedia.map(m => typeof m === 'string' ? { url: m, type: 'image' } : m), ...newMedia]; const data = { ...newItem, price: Number(newItem.price), stock: Number(newItem.stock), media: finalMedia, image: finalMedia[0]?.url, sellerId: id, sellerName: page.name, sellerAvatar: page.logo, isBusiness: true, timestamp: serverTimestamp() }; if (isEditingProduct) await updateDoc(doc(db, 'marketplace', currentProductId), data); else await addDoc(collection(db, 'marketplace'), { ...data, boost: null, reviews: [], questions: [], sold: false }); setShowProductModal(false); } catch (err) { alert("Failed."); } finally { setProductUploading(false); } };
  const handleDeleteProduct = async (p) => { if(confirm("Delete?")) { await deleteDoc(doc(db, 'marketplace', p.id)); setActiveMenuId(null); } };
  const handleBoost = async (c) => { await updateDoc(doc(db, c.targetType === 'page' ? 'business_pages' : 'marketplace', c.targetId), { boost: { ...c, expiresAt: serverTimestamp() } }); alert("Boosted!"); setShowBoostModal(false); };
  ...
  return (
    <div className="p-0 md:p-6 w-full max-w-[1400px] mx-auto pb-24">
        ...
        {/* HEADER */}
        <div className="bg-white md:rounded-[30px] shadow-sm mb-6 relative group overflow-hidden">
            ...
            <div className="px-6 md:px-10 pb-6 -mt-16 relative z-10 flex flex-col md:flex-row items-end gap-6">
                 ...
                 <div className="flex-1 text-center md:text-left mb-2">
                     <h1 className="text-3xl font-black text-dark">{page.name}</h1>
                 ...
                 <div className="flex gap-3 mb-2 justify-center w-full md:w-auto">
                     {!isTeam ? (
                         ...
                         <>
                             {isAdmin && <button onClick={() => { setBoostTarget({id, type:'page', name:page.name}); setShowBoostModal(true); }} className="bg-gold text-white px-5 py-3 rounded-xl font-bold shadow-lg hover:bg-yellow-600 transition"><i className="fas fa-rocket mr-2"></i> Boost</button>}
                                ...
                                <div key={p.id} className="bg-white p-3 rounded-2xl shadow-sm border border-gray-100 relative group">
                                    <div onClick={() => navigate(`/product/${p.id}`)} className="cursor-pointer">
                                        <div className="relative rounded-xl overflow-hidden mb-3 aspect-square">
                                            {p.media?.[0]?.type === 'video' ? <video src={p.media[0].url} className="w-full h-full object-cover" /> : <img src={p.image} className="w-full h-full object-cover" alt={p.title} />}
                ...
                {activeTab === 'team' && isTeam && (
                    <div className="bg-white p-6 rounded-[24px] shadow-sm border border-gray-100">
                        ...
                        <div className="space-y-4">
                            {page.team?.map(member => (
                                <div key={member.uid} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        {page.ownerId === member.uid ? (
                                            <span className="text-primary font-bold text-sm ml-2">Owner</span>
                                        ) : (
                                            isAdmin && (
                            ...
                            {page.team?.length === 0 && <p className="text-gray-400 text-center py-4">No team members yet.</p>}
                        </div>
                ...
                {activeTab === 'growth' && isAdmin && (
                    <div className="space-y-6">
                        ...
                        <div className="bg-gradient-to-br from-dark to-gray-900 p-6 rounded-[24px] text-white shadow-lg">
                            ...
                            <button onClick={() => { setBoostTarget({id, type:'page', name:page.name}); setShowBoostModal(true); }} className="w-full bg-white text-dark font-bold py-3 rounded-xl hover:bg-gold transition-colors">
        ...
        {/* EDIT PAGE MODAL (Reusing a simpler structure for brevity) */}
        {showEditPageModal && (
        ...
        {/* PRODUCT MODAL (Reusing a simpler structure for brevity) */}
        {showProductModal && (<div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[9999] p-4">
            <div className="bg-white w-full max-w-lg rounded-[30px] p-6 max-h-[90vh] overflow-y-auto">
                ...
                <select value={newItem.category} onChange={e=>setNewItem({...newItem,category:e.target.value})} className="w-full border p-3 rounded-xl mb-2">
                ...
                <div className="mb-4 flex flex-wrap gap-2">
                    {[...existingMedia, ...mediaFiles.map(f => ({ url: URL.createObjectURL(f), file: f, isNew: true }))].map((media, index) => (
                        <div key={index} className="relative w-16 h-16 rounded-lg overflow-hidden border border-gray-300">
                            ...
                            <button onClick={() => { if (media.isNew) setMediaFiles(mediaFiles.filter(f => f !== media.file)); else setExistingMedia(existingMedia.filter((_, i) => i !== index)); }} className="absolute top-0 right-0 bg-red-500 text-white rounded-full w-4 h-4 text-[8px] flex items-center justify-center"><i className="fas fa-times"></i></button>
                        </div>
                    ))}
                </div>
