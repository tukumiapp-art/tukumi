    ...
    const unsubAuth = onAuthStateChanged(auth, (currentUser) => {
      ...
      if (currentUser) {
            ...
            if (change.type === 'added' && (call.status === 'calling' || call.status === 'ringing')) {
                ...
                if (playPromise !== undefined) {
                    playPromise.catch(error => {
                        console.log("Ringtone auto-play blocked. waiting for interaction.");
                    });
                }
                
                // Tell caller we received it
                if (call.status === 'calling') {
                    updateDoc(doc(db, 'calls', call.id), { status: 'ringing' });
                }
            }
            
            // Modified: Ended/Rejected
        ...
        const unsubMsgs = onSnapshot(qMsgs, (snapshot) => {
            snapshot.docChanges().forEach((change) => {
                if (change.type === 'modified' && change.doc.data().lastMessageSenderId !== currentUser.uid) {
                    const data = change.doc.data();
                    // Only show if recent (last 5 seconds)
                    if (data.updatedAt?.seconds > Date.now()/1000 - 5) {
                        ...
                        setMessageNotification({
                            id: change.doc.id,
                            senderName: sender?.displayName || 'Chat',
  ...
  const handleAnswer = async () => {
      ...
      navigate('/messages', { 
          state: { 
              ...
              incomingCallData: { ...incomingCall, status: 'connected' } 
          } 
      });
      setIncomingCall(null);
  };
    ...
    <div className="fixed top-0 left-0 right-0 z-[9999] pointer-events-none flex flex-col items-center pt-4 px-4">
      {incomingCall && (
        <div className="pointer-events-auto w-full max-w-sm bg-dark/95 backdrop-blur-md text-white p-4 rounded-2xl shadow-2xl flex items-center gap-4 animate-slide-down border border-white/10">
           <div className="relative">
             <img src={incomingCall.callerAvatar || "https://via.placeholder.com/50"} className="w-14 h-14 rounded-full border-2 border-green-500 animate-pulse" />
           ...
           <div className="flex-1 min-w-0">
             ...
             <p className="text-xs text-gray-300 flex items-center gap-1">
                <i className={`fas ${incomingCall.type === 'video' ? 'fa-video' : 'fa-phone-alt'}`}></i> 
                Incoming {incomingCall.type} call...
             </p>
           </div>
           <div className="flex gap-3">
             <button onClick={handleDecline} className="w-10 h-10 rounded-full bg-red-500 hover:bg-red-600 flex items-center justify-center transition-transform hover:scale-110"><i className="fas fa-times"></i></button>
             <button onClick={handleAnswer} className="w-10 h-10 rounded-full bg-green-500 hover:bg-green-600 flex items-center justify-center transition-transform hover:scale-110 animate-bounce"><i className="fas fa-phone"></i></button>
           </div>
        </div>
      )}
      {messageNotification && !incomingCall && (
        <div onClick={handleMessageClick} className="pointer-events-auto mt-2 bg-white/90 backdrop-blur-xl p-3 rounded-2xl shadow-lg border border-gray-100 flex items-center gap-3 w-full max-w-sm cursor-pointer animate-fade-in-up">
            <img src={messageNotification.senderAvatar} className="w-10 h-10 rounded-full object-cover" />
            <div className="flex-1 min-w-0">
            ...
            <div className="w-6 h-6 bg-primary rounded-full flex items-center justify-center text-[10px] text-white font-bold">1</div>
        </div>
      )}
    </div>
  );
