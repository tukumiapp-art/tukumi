...
const MobileNav = () => {
    ...
    const unsubAuth = onAuthStateChanged(auth, (currentUser) => {
        if (currentUser) {
            ...
            const unsubMsgs = onSnapshot(qMsgs, (snapshot) => {
                let total = 0;
                snapshot.docs.forEach(doc => { const data = doc.data(); if (data.unreadCounts && data.unreadCounts[currentUser.uid]) total += data.unreadCounts[currentUser.uid]; });
  ...
  useEffect(() => {
    ...
    document.addEventListener('focusin', handleFocusIn); document.addEventListener('focusout', handleFocusOut); window.addEventListener('toggle-nav', handleToggleNav);
    setIsVisible(true);
    return () => { document.removeEventListener('focusin', handleFocusIn); document.removeEventListener('focusout', handleFocusOut); window.removeEventListener('toggle-nav', handleToggleNav); };
  }, [location.pathname]);
